...
description: Audit and subtractively clean Skills, AGENTS.md, and coding-agent scaffolding for GPT-6 Astra. Use when Astra stops early, loads the wrong skills, over-reads, over-tests, over-asks, or when migrating Sol-era harness instructions—not for rewriting product prompts (use optimize-gpt-6-astra-prompts).
---

# Audit GPT-6 Astra Skills and Agent Scaffold

Audit repository Skills, `AGENTS.md`, and task prompts for GPT-6 Astra. Prefer subtractive cleanup over adding rules. Preserve intentional safety boundaries; remove obsolete handholding that now causes pauses, wrong skill loads, over-testing, or context burn.

## Sources

Read [references/practitioner-guidance.md](references/practitioner-guidance.md) for the checklist and bad/good examples. Canonical source: Eric Provencher, [Rethinking skills and prompts for GPT-6 Astra](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra). For copy-ready product prompts and API migration, use `$optimize-gpt-6-astra-prompts` / [Using GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model).
...
- skill directories (`SKILL.md` frontmatter descriptions, bodies, bundled scripts/references);
...
Ask only if the audit target is unclear. Otherwise proceed.
...
| --- | --- |
| Wrong or extra skills load | Long, overlapping, contradictory, or "pick me" descriptions; description truncation under many skills |
| Early pause or redundant approval | Strong ask-first language; unclear completion; skill vs user conflicts |
...
3. **Proposed edit** (delete, shorten, narrow trigger, router-ize, or replace with a permission/completion rule).
...
- Keep `description` short and trigger-specific; front-load the job so truncation still leaves a usable signal.
- Prefer progressive disclosure: root `SKILL.md` as a minimal router; detail in supporting files loaded only when needed.
...
- If the same skill serves Sol/Luna and Astra, note where guidance overconstrains Astra; suggest model-specific splits only when needed.

**AGENTS.md**

- Drop blanket "read the whole repo / stack of docs before every edit."
- Keep contextual doc pointers; remove mandatory pre-read rituals.
...
- Revisit strong ask-first language: keep it for irreversible/external actions; remove it for reversible in-scope work. Astra is highly aligned—treat blanket "ask first" as a risk of early stops.
...
- Keep "stop after first implementation for review" only when that checkpoint is intentional.
